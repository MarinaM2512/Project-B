function [data_template_x, time_template_x]= ...
               make_tamplate_all_movments_x (...
                dates,names,delay,thresh_time,thresh,sum_window,border_factor)
% function returns avgraged template of gyro x for all measurments of
% the same movement. 
...
% Output Arguments:
% 1. data_template_x - data template vector for all mes of the selected movement
% 2. time_template_x - matching time vector
        
d_max = length(dates);
n_max = length(names);
cell_avgX =cell(1,d_max*n_max);
i = 1;
for d = 1:d_max
    for n = 1:n_max
        [cell_avgX{1,i}, ~, ~] = ...
        join_measurments_of_movements(dates(d),names(n),delay,thresh_time,thresh,sum_window);
        i = i+1;               
    end
end
to_avgX = combine_to_matrix(cell_avgX);
[data_template_x , time_template_x] = make_template(to_avgX,border_factor);
end
