function [data_template_y, time_template_y]= ...
               make_tamplate_all_movments_y (...
                dates,names,delay,thresh_time,thresh,sum_window,border_factor)
% function returns avgraged template of gyro y for all measurments of
% the same movement. 
...
% Output Arguments:
% 1. data_template_y - data template vector for all mes of the selected movement
% 2. time_template_y - matching time vector
        
d_max = length(dates);
n_max = length(names);
cell_avgY =cell(1,d_max*n_max);
i = 1;
for d = 1:d_max
    for n = 1:n_max
        [ ~ , cell_avgY{1,i} , ~ ] = ...
...
to_avgY = combine_to_matrix(cell_avgY);

    [data_template_y , time_template_y] = make_template(...
                                                to_avgY,border_factor);
end
