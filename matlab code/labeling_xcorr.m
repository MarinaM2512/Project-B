function labels = labeling_xcorr(xcorr_thresholded,times,thresh_val,hold_time)
...
% 3. check if in the period of time marked as hold_time before the current
...
%init 
labels = zeros(length(xcorr_thresholded),1);
label_values =  zeros(length(xcorr_thresholded),1);
...
for i=1:4
    thresholded(:,i) =  xcorr_weighted(:,i).*(xcorr_weighted > thresh_val);
end
for i = 1: length(thresholded)
    peak_ind = find(thresholded(i,:));
    if(~isempty(peak_ind))
        ...
        if(i>1)
            ...
            is_larger = 0;
            for j = 1:4
                is_larger = is_larger || ~isempty(find(label_values(i-hold:i-1,j)>curr_max));
            end
            [max_val, max_ind]
